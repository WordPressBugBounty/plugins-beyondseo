...
use BeyondSEODeps\Symfony\Bridge\Doctrine\ContainerAwareEventManager;
...
class RegisterEventListenersAndSubscribersPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($listenerRefs as $connection => $refs) {
            $this->getEventManagerDef($container, $connection)->replaceArgument(0, ServiceLocatorTagPass::register($container, $refs));
        }
    }
    private function addTaggedServices(ContainerBuilder $container): array
    {
        ...
        foreach ($taggedServices as $taggedSubscriber) {
            ...
            $connections = isset($tag['connection']) ? [$container->getParameterBag()->resolveValue($tag['connection'])] : array_keys($this->connections);
