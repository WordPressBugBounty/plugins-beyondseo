...
use BeyondSEODeps\Composer\Composer;
use BeyondSEODeps\Composer\EventDispatcher\EventSubscriberInterface;
use BeyondSEODeps\Composer\Factory;
use BeyondSEODeps\Composer\IO\IOInterface;
use BeyondSEODeps\Composer\Plugin\PluginInterface;
use BeyondSEODeps\Composer\Script\ScriptEvents;
...
class ComposerPlugin implements PluginInterface, EventSubscriberInterface
{
    ...
    public function updateAutoloadFile(): void
    {
        ...
        if (!is_file($autoloadFile = $vendorDir . '/autoload.php') || false === $extra = $this->composer->getPackage()->getExtra()['runtime'] ?? []) {
            return;
        }
        $fs = new Filesystem();
        ...
        $code = strtr(file_get_contents($autoloadTemplate), ['%project_dir%' => $projectDir, '%runtime_class%' => var_export($runtimeClass, true), '%runtime_options%' => '[' . substr(var_export($extra, true), 7, -1) . "  'project_dir' => {$projectDir},\n]"]);
    ...
    public static function getSubscribedEvents(): array
    {
        ...
        return [ScriptEvents::POST_AUTOLOAD_DUMP => 'updateAutoloadFile'];
    }
}