...
namespace BeyondSEODeps\GuzzleHttp;

use BeyondSEODeps\GuzzleHttp\Cookie\CookieJar;
use BeyondSEODeps\GuzzleHttp\Exception\GuzzleException;
use BeyondSEODeps\GuzzleHttp\Exception\InvalidArgumentException;
use BeyondSEODeps\GuzzleHttp\Promise as P;
use BeyondSEODeps\GuzzleHttp\Promise\PromiseInterface;
...
class Client implements ClientInterface, \Psr\Http\Client\ClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $uri = $this->buildUri(\BeyondSEODeps\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \BeyondSEODeps\GuzzleHttp\Psr7\UriResolver::resolve(\BeyondSEODeps\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = \BeyondSEODeps\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \BeyondSEODeps\GuzzleHttp\Psr7\MultipartStream) {
