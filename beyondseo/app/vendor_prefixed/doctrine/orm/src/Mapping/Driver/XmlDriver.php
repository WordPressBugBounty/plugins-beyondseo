...
use BeyondSEODeps\Doctrine\Common\Collections\Order;
use BeyondSEODeps\Doctrine\ORM\Mapping\Builder\EntityListenerBuilder;
use BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata;
use BeyondSEODeps\Doctrine\ORM\Mapping\MappingException;
use BeyondSEODeps\Doctrine\Persistence\Mapping\ClassMetadata as PersistenceClassMetadata;
use BeyondSEODeps\Doctrine\Persistence\Mapping\Driver\FileDriver;
use DOMDocument;
...
class XmlDriver extends FileDriver
{
    ...
    public function loadMetadataForClass($className, PersistenceClassMetadata $metadata)
    {
        ...
        if (isset($xmlRoot['inheritance-type'])) {
            ...
            $metadata->setInheritanceType(constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::INHERITANCE_TYPE_' . $inheritanceType));
        ...
        if (isset($xmlRoot['change-tracking-policy'])) {
            $metadata->setChangeTrackingPolicy(constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CHANGETRACKING_'
        ...
        foreach ($xmlRoot->id ?? [] as $idElement) {
            ...
            if (isset($idElement->generator)) {
                ...
                $metadata->setIdGeneratorType(constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::GENERATOR_TYPE_'
                    . $strategy));
            }
        ...
        if (isset($xmlRoot->{'one-to-one'})) {
            foreach ($xmlRoot->{'one-to-one'} as $oneToOneElement) {
                ...
                if (isset($oneToOneElement['fetch'])) {
                    $mapping['fetch'] = constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . (string) $oneToOneElement['fetch']);
                }
        ...
        if (isset($xmlRoot->{'one-to-many'})) {
            foreach ($xmlRoot->{'one-to-many'} as $oneToManyElement) {
                ...
                if (isset($oneToManyElement['fetch'])) {
                    $mapping['fetch'] = constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . (string) $oneToManyElement['fetch']);
                }
        ...
        if (isset($xmlRoot->{'many-to-one'})) {
            foreach ($xmlRoot->{'many-to-one'} as $manyToOneElement) {
                ...
                if (isset($manyToOneElement['fetch'])) {
                    $mapping['fetch'] = constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . (string) $manyToOneElement['fetch']);
                }
        ...
        if (isset($xmlRoot->{'many-to-many'})) {
            foreach ($xmlRoot->{'many-to-many'} as $manyToManyElement) {
                ...
                if (isset($manyToManyElement['fetch'])) {
                    $mapping['fetch'] = constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . (string) $manyToManyElement['fetch']);
                }
        ...
        if (isset($xmlRoot->{'lifecycle-callbacks'})) {
            foreach ($xmlRoot->{'lifecycle-callbacks'}->{'lifecycle-callback'} ?? [] as $lifecycleCallback) {
                $metadata->addLifecycleCallback((string) $lifecycleCallback['method'], constant('BeyondSEODeps\Doctrine\ORM\Events::' . (string) $lifecycleCallback['type']));
            }
        }
    ...
    private function columnToArray(SimpleXMLElement $fieldMapping): array
    {
        ...
        if (isset($fieldMapping['generated'])) {
            $mapping['generated'] = constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::GENERATED_' . (string) $fieldMapping['generated']);
        }
    ...
    private function cacheToArray(SimpleXMLElement $cacheMapping): array
    {
        ...
        if ($usage && ! defined('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CACHE_USAGE_' . $usage)) {
            throw new InvalidArgumentException(sprintf('Invalid cache usage "%s"', $usage));
        }

        if ($usage) {
            $usage = (int) constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CACHE_USAGE_' . $usage);
        }
