...
use BeyondSEODeps\Doctrine\Persistence\Mapping\Driver\ColocatedMappingDriver;
use LogicException;
use ReflectionClass;
...
class AttributeDriver extends CompatibilityAnnotationDriver
{
    ...
    public function loadMetadataForClass($className, PersistenceClassMetadata $metadata): void
    {
        ...
        if (isset($classAttributes[Mapping\Cache::class])) {
            ...
            $cacheMap       = [
                ...
                'usage'  => constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CACHE_USAGE_' . $cacheAttribute->usage),
            ];
        ...
        if (isset($classAttributes[Mapping\InheritanceType::class])) {
            ...
            $metadata->setInheritanceType(
                constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::INHERITANCE_TYPE_' . $inheritanceTypeAttribute->value)
            );
        ...
        if (isset($classAttributes[Mapping\ChangeTrackingPolicy::class])) {
            ...
            $metadata->setChangeTrackingPolicy(constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CHANGETRACKING_' . $changeTrackingAttribute->value));
        }
        ...
        foreach ($reflectionClass->getProperties() as $property) {
            ...
            if ($cacheAttribute !== null) {
                $mapping['cache'] = $metadata->getAssociationCacheDefaults(
                    $mapping['fieldName'],
                    [
                        'usage'  => (int) constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::CACHE_USAGE_' . $cacheAttribute->usage),
                        'region' => $cacheAttribute->region,
                    ]
                );
            }
            ...
            if ($columnAttribute !== null) {
                ...
                if ($generatedValueAttribute !== null) {
                    $metadata->setIdGeneratorType(constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::GENERATOR_TYPE_' . $generatedValueAttribute->strategy));
                }
    ...
    {
        if (! defined('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . $fetchMode)) {
            throw MappingException::invalidFetchMode($className, $fetchMode);
        }

        return constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::FETCH_' . $fetchMode);
    }
    ...
    {
        if (! defined('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::GENERATED_' . $generatedMode)) {
            throw MappingException::invalidGeneratedMode($generatedMode);
        }

        return constant('BeyondSEODeps\Doctrine\ORM\Mapping\ClassMetadata::GENERATED_' . $generatedMode);
    }
