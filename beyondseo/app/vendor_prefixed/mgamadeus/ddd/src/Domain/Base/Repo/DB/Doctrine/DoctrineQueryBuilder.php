...
use BeyondSEODeps\Doctrine\Common\Collections\Criteria;
...
use BeyondSEODeps\Doctrine\ORM\Query\Expr;
use BeyondSEODeps\Doctrine\ORM\Query\Parameter;
use BeyondSEODeps\Doctrine\ORM\Query\QueryExpressionVisitor;
use BeyondSEODeps\Doctrine\ORM\QueryBuilder;
...
class DoctrineQueryBuilder extends QueryBuilder
{
     ...
     */
    protected $dqlParts = ['distinct' => false, 'select' => [], 'from' => [], 'join' => [], 'set' => [], 'where' => null, 'groupBy' => [], 'having' => null, 'orderBy' => []];
    /**
    ...
    public function getMappedResult(Result $result): mixed
    {
        ...
        return $this->getEntityManager()->newHydrator($query->getHydrationMode())->hydrateAll($result, $rsm, $query->getHints());
    }
    /**
    ...
    public function applyDistinctSubqueryIfNeededAndGetResult(): mixed
    {
        ...
        $subquerySQL = preg_replace('/^\s*SELECT.*?FROM/', 'SELECT DISTINCT ' . $mainEntityAlias . 'sub.id FROM', $subquerySQL);
    ...
    public function indexBy($alias, $indexBy)
    {
        ...
        foreach ($this->dqlParts['from'] as &$fromClause) {
            assert($fromClause instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\From);
    ...
    public function innerJoin($join, $alias, $conditionType = null, $condition = null, $indexBy = null)
    {
        ...
        $join = new Expr\Join(\BeyondSEODeps\Doctrine\ORM\Query\Expr\Join::INNER_JOIN, $join, $alias, $conditionType, $condition, $indexBy);
    ...
    public function leftJoin($join, $alias, $conditionType = null, $condition = null, $indexBy = null)
    {
        $parentAlias = substr($join, 0, (int) strpos($join, '.'));
        $rootAlias = $this->findRootAlias($alias, $parentAlias);
        $join = new Expr\Join(\BeyondSEODeps\Doctrine\ORM\Query\Expr\Join::LEFT_JOIN, $join, $alias, $conditionType, $condition, $indexBy);
        return $this->add('join', [$rootAlias => $join], true);
    }
    /**
    ...
    {
        return $this->add('set', new Expr\Comparison($key, \BeyondSEODeps\Doctrine\ORM\Query\Expr\Comparison::EQ, $value), true);
    }
    /**
    ...
    {
        if (!(func_num_args() === 1 && $predicates instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Composite)) {
    ...
    public function andWhere()
    {
        ...
        if ($where instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Andx) {
    ...
    public function orWhere()
    {
        $args = func_get_args();
        $where = $this->getDQLPart('where');
        if ($where instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Orx) {
            $where->addMultiple($args);
        } else {
            array_unshift($args, $where);
    ...
    {
        if (!(func_num_args() === 1 && ($having instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Andx || $having instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Orx))) {
    ...
    public function andHaving($having)
    {
        ...
        if ($having instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Andx) {
    ...
    public function orHaving($having)
    {
        $args = func_get_args();
        $having = $this->getDQLPart('having');
        if ($having instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\Orx) {
            $having->addMultiple($args);
        } else {
            array_unshift($args, $having);
    ...
    {
        $orderBy = $sort instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\OrderBy ? $sort : new Expr\OrderBy($sort, $order);
    ...
    {
        $dql = 'SELECT' . ($this->dqlParts['distinct'] === true ? ' DISTINCT' : '') . $this->getReducedDQLQueryPart('select', ['pre' => ' ', 'separator' => ', ']);
        ...
        if (!empty($fromParts)) {
            ...
            foreach ($fromParts as $from) {
                ...
                if ($from instanceof \BeyondSEODeps\Doctrine\ORM\Query\Expr\From && isset($joinParts[$from->getAlias()])) {
