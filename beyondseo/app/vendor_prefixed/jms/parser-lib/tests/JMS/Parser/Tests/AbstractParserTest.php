...
namespace BeyondSEODeps\JMS\Parser\Tests;
...
class AbstractParserTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @expectedException \BeyondSEODeps\JMS\Parser\SyntaxErrorException
    ...
    {
        $this->lexer = $lexer = new \BeyondSEODeps\JMS\Parser\SimpleLexer('/([0-9]+)|\s+|(.)/', array(0 => 'T_UNKNOWN', 1 => 'T_INT', 100 => 'T_PLUS', 101 => 'T_MINUS'), function ($value) {
            if ('+' === $value) {
            ...
            if (is_numeric($value)) {
                return array(AbstractParserTest::T_INT, (int) $value);
            }
            return AbstractParserTest::T_UNKNOWN;
        });
        $this->parser = $parser = $this->getMockBuilder('BeyondSEODeps\JMS\Parser\AbstractParser')->setConstructorArgs(array($this->lexer))->getMockForAbstractClass();
        $match = function ($type) use ($parser) {
            $ref = new \ReflectionMethod($parser, 'match');
            $ref->setAccessible(true);
            return $ref->invoke($parser, $type);
        };
        $this->parser->expects($this->any())->method('parseInternal')->will($this->returnCallback(function () use ($lexer, $match) {
