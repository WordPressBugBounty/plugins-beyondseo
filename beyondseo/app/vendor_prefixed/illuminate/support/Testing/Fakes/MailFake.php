...
use Closure;
use BeyondSEODeps\Illuminate\Contracts\Mail\Factory;
use BeyondSEODeps\Illuminate\Contracts\Mail\Mailable;
use BeyondSEODeps\Illuminate\Contracts\Mail\Mailer;
use BeyondSEODeps\Illuminate\Contracts\Mail\MailQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = beondseodeps_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = beondseodeps_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return beondseodeps_collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return beondseodeps_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Contracts\Mail\Mailable|string|array  $view
     * @param  array  $data
