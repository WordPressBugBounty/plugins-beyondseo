...
namespace BeyondSEODeps\Illuminate\Support\Testing\Fakes;

use Closure;
use BeyondSEODeps\Illuminate\Bus\PendingBatch;
use BeyondSEODeps\Illuminate\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements QueueingDispatcher
{
     ...
     *
     * @var \BeyondSEODeps\Illuminate\Contracts\Bus\QueueingDispatcher
     */
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Contracts\Bus\QueueingDispatcher  $dispatcher
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            beondseodeps_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return beondseodeps_tap(clone $job, function ($job) {
    ...
    {
        $chain = beondseodeps_collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return beondseodeps_collect($chain)->map(function ($job) {
    ...
    {
        return ! beondseodeps_collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return beondseodeps_collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return beondseodeps_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return beondseodeps_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return beondseodeps_collect($this->batches)->filter(function ($batch) use ($callback) {
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Support\Collection|array  $jobs
     ...
     * @return \BeyondSEODeps\Illuminate\Bus\Batch|null
     */
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Support\Collection|array  $jobs
     * @return \BeyondSEODeps\Illuminate\Bus\PendingBatch
     */
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Bus\PendingBatch  $pendingBatch
     * @return \BeyondSEODeps\Illuminate\Bus\Batch
     */
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return beondseodeps_collect($this->jobsToFake)
