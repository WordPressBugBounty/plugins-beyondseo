...
use BeyondSEODeps\Illuminate\Contracts\Foundation\Application;
use BeyondSEODeps\Illuminate\Contracts\View\Factory;
use BeyondSEODeps\Illuminate\Filesystem\Filesystem;
use BeyondSEODeps\Illuminate\Support\Str;
use BeyondSEODeps\Illuminate\View\AnonymousComponent;
use BeyondSEODeps\Illuminate\View\DynamicComponent;
use BeyondSEODeps\Illuminate\View\ViewFinderInterface;
...
class ComponentTagCompiler
{
     ...
     *
     * @var \BeyondSEODeps\Illuminate\View\Compilers\BladeCompiler
     */
    protected $blade;
     ...
     * @param  \BeyondSEODeps\Illuminate\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
    ...
    public function partitionDataAndAttributes($class, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            return [beondseodeps_collect($attributes), beondseodeps_collect($attributes)];
        }

        $constructor = (new ReflectionClass($class))->getConstructor();

        $parameterNames = $constructor
                    ? beondseodeps_collect($constructor->getParameters())->map->getName()->all()
                    : [];

        return beondseodeps_collect($attributes)->partition(function ($value, $key) use ($parameterNames) {
    ...
    protected function getAttributesFromAttributeString(string $attributeString)
    {
        ...
        return beondseodeps_collect($matches)->mapWithKeys(function ($match) {
    ...
    {
        return beondseodeps_collect(token_get_all($value))->map(function ($token) {
            if (! is_array($token)) {
                return $token;
            }
    ...
    {
        return beondseodeps_collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \BeyondSEODeps\Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
