...
namespace BeyondSEODeps\Illuminate\Bus;

use Carbon\CarbonImmutable;
use Closure;
use BeyondSEODeps\Illuminate\Contracts\Queue\Factory as QueueFactory;
...
class Batch implements Arrayable, JsonSerializable
{
     ...
     *
     * @var \BeyondSEODeps\Illuminate\Contracts\Queue\Factory
     */
    protected $queue;
     ...
     *
     * @var \BeyondSEODeps\Illuminate\Bus\BatchRepository
     */
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Contracts\Queue\Factory  $queue
     * @param  \BeyondSEODeps\Illuminate\Bus\BatchRepository  $repository
     * @param  string  $id
     * @param  string  $name
     * @param  int  $totalJobs
     ...
     *
     * @param  \BeyondSEODeps\Illuminate\Support\Enumerable|array  $jobs
     * @return self
     */
    public function add($jobs)
    {
        ...
        $jobs = Collection::wrap($jobs)->map(function ($job) use (&$count) {
            ...
            if (is_array($job)) {
                ...
                return beondseodeps_with($this->prepareBatchedChain($job), function ($chain) {
                    return $chain->first()
    ...
    public function recordSuccessfulJob(string $jobId)
    {
        ...
        if ($counts->pendingJobs === 0 && $this->hasThenCallbacks()) {
            ...
            beondseodeps_collect($this->options['then'])->each(function ($handler) use ($batch) {
        ...
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            ...
            beondseodeps_collect($this->options['finally'])->each(function ($handler) use ($batch) {
                $this->invokeHandlerCallback($handler, $batch);
            });
        }
    }
     ...
     *
     * @param  string  $jobId
     * @return \BeyondSEODeps\Illuminate\Bus\UpdatedBatchJobCounts
     */
    ...
    public function recordFailedJob(string $jobId, $e)
    {
        ...
        if ($counts->failedJobs === 1 && $this->hasCatchCallbacks()) {
            ...
            beondseodeps_collect($this->options['catch'])->each(function ($handler) use ($batch, $e) {
        ...
        if ($counts->allJobsHaveRanExactlyOnce() && $this->hasFinallyCallbacks()) {
            ...
            beondseodeps_collect($this->options['finally'])->each(function ($handler) use ($batch, $e) {
                $this->invokeHandlerCallback($handler, $batch, $e);
            });
        }
    }
